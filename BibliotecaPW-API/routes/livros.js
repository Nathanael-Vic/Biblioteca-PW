...
router.delete('/acervo/:id', async (req, res) => {
    ...
    try {
        ...
        const [emprestimosAtivos] = await connection.execute("SELECT id FROM emprestimos WHERE livro_id = ? AND status = 'ativo'", [id]);
        if (emprestimosAtivos.length > 0) {
            return res.status(400).json({ mensagem: 'Não é possível excluir o livro, pois ele está atualmente emprestado.' });
        }

        await connection.beginTransaction();

        await connection.execute('DELETE FROM emprestimos WHERE livro_id = ?', [id]);
        ...
        res.status(200).json({ mensagem: `Livro com ID ${id} e seu histórico foram removidos com sucesso.` });
        
    } catch (error) {
        if (connection) await connection.rollback();
        console.error(`Erro ao deletar o livro ${id}:`, error);
        res.status(500).json({ mensagem: 'Erro interno no servidor ao tentar excluir o livro.' });
    } finally {
        if (connection) connection.release();
    }
});
